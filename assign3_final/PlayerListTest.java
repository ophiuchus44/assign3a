...
public class PlayerListTest
{
	public static void main(String [] args) throws IOException
	{
		...
		PlayerList allPlayers = new PlayerList("playersTest.txt");
		...
		for (String S: names)
		{
			...
			Player onePlayer = allPlayers.getPlayer(S.toLowerCase());
			if (onePlayer != null)
			{
			...
			else
			{
				...
				// fields should be 0
				onePlayer = new Player(S.toLowerCase());
		...
		for (int i = 0; i < 2; i++)
		{
			...
			Player P = allPlayers.removePlayer(rName.toLowerCase());
			if (P != null)
			{
